...
describe('Spin', () => {
  it('renders spinner with spinning class when spinning=true', () => {
    ...
    expect(container.querySelector('.fi-spin-spinning')).toBeInTheDocument();
  });

  it('does not apply spinning class when spinning=false', () => {
    const { container } = render(<Spin spinning={false} />);
    expect(container.querySelector('.fi-spin-spinning')).not.toBeInTheDocument();
  });

  it('applies size class correctly', () => {
    const { container } = render(<Spin size="small" />);
    expect(container.querySelector('.fi-spin-sm')).toBeInTheDocument();
  });
