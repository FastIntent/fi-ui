...
describe('Pagination Component', () => {
  it('renders without crashing', () => {
    ...
    expect(container.querySelector('.atom-pagination')).toBeInTheDocument();
  });
  ...
  it('renders the correct number of page items for total=50 pageSize=10', () => {
    ...
    // 5 page items
    expect(container.querySelectorAll('.atom-pagination-item')).toHaveLength(5);
  });
  ...
  it('calls onChange when a page item is clicked', () => {
    ...
    const page2 = container.querySelector('.atom-pagination-item-2') as HTMLElement;
  ...
  it('applies small size class', () => {
    ...
    expect(container.querySelector('.atom-pagination-small')).toBeInTheDocument();
  });

  it('applies start alignment class by default', () => {
    const { container } = render(<Pagination total={50} />);
    expect(container.querySelector('.atom-pagination-start')).toBeInTheDocument();
  });

  it('applies center alignment class', () => {
    const { container } = render(<Pagination total={50} align="center" />);
    expect(container.querySelector('.atom-pagination-center')).toBeInTheDocument();
  });
});
