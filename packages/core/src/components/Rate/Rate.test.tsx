...
describe('Rate Component', () => {
  it('renders the correct number of stars (default 5)', () => {
    ...
    const stars = container.querySelectorAll('.atom-rate-star');
  ...
  it('applies base class', () => {
    const { container } = render(<Rate />);
    expect(container.querySelector('.atom-rate')).toBeInTheDocument();
  });

  it('applies disabled class when disabled', () => {
    const { container } = render(<Rate disabled />);
    expect(container.querySelector('.atom-rate-disabled')).toBeInTheDocument();
  });

  it('marks stars as full up to the defaultValue', () => {
    const { container } = render(<Rate defaultValue={3} count={5} />);
    const fullStars = container.querySelectorAll('.atom-rate-star-full');
